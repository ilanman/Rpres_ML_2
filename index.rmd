...
```{r curve, echo=FALSE, message=FALSE, results='hide', fig.height=4, fig.width=4}
...
```{r using_optimize,message=FALSE,comment="",warning=FALSE,results='hide' }
...
```{r using_optimize_ans,message=FALSE,comment="",warning=FALSE}
optimize(fn,c(-100,100))  ## built-in R optimization function
```
...
$\bf{Ax} = \bf{\lambda x}$
```{r eigen_proof, comment="",eval=TRUE,results='hide'}
x1 = c(1,-1)
x2 = c(1,1)
A %*% x1 == 3 * x1
A %*% x2 == 7 * x2
...
par(new=TRUE)
plot(var_explained$PC, var_explained$cum_var_explained,type="l",col="blue",xaxt="n",yaxt="n",xlab="Principal Component",ylab="Variance Explained",ylim=c(0,100))
axis(4)
