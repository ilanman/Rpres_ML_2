...
newton <-function(data,dll,d2ll,init,eps=1e-6,maxiter=20)
{
#
# Use the Newton-Raphson algorithm to get the mle.
# Input is the data, functions to get the first and
# second derivatives of the log-likelihood and an
...
# value of score function
  ...
  while (continue) {
      i <- i+1
      theta.old <- theta
      theta <- theta - dlt/d2ll(theta,data) ## NR update
      dlt <- dll(theta,data)
      out[i,] <- c(theta,dlt)
